...
public sealed class ImageTransform(Stream stream) : IDisposable
{
    ...
    {
        _image.Resize(new MagickGeometry((uint)width, (uint)height));
        return this;
    }
    ...
    public ImageTransform RemoveMetadata()
    {
        ...
        if (exifProfile != null)
            _image.RemoveProfile(exifProfile);
        if (iptcProfile != null)
            _image.RemoveProfile(iptcProfile);
        if (xmpProfile != null)
            _image.RemoveProfile(xmpProfile);
        return this;
    }
